    ...
    internal class GroundTruthAmbientOcclusion : ScriptableRendererFeature
    {
        ...
        private class ScreenSpaceAmbientOcclusionPass : ScriptableRenderPass
        {
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                Vector2 focalLen = new Vector2(invHalfTanFov * (((float)renderingData.cameraData.camera.pixelHeight / (float)downsampleDivider) / ((float)renderingData.cameraData.camera.pixelWidth / (float)downsampleDivider)), invHalfTanFov);
                Vector2 invFocalLen = new Vector2(1 / focalLen.x, 1 / focalLen.y);

                m_Material.SetVector(SSAO_UVToView_ID, new Vector4(2 * invFocalLen.x, 2 * invFocalLen.y, -1 * invFocalLen.x, -1 * invFocalLen.y));
